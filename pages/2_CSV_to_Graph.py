...
def elo_change_history():
    try:
        history = st.file_uploader("Upload history.csv")
        elo = st.file_uploader("Upload elo.csv")
        if history is not None and elo is not None:
            df = pd.read_csv(history)
            df.index = df.index + 1
            player_df = pd.read_csv(elo).set_index('Player')
